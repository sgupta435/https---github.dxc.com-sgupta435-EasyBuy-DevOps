...
var ezb_s_account="hphqglobal,hphqemea,devhphqeasybuyv2";// default account
var s_account;
...
function SetEZBaccount(){
	...
	if(_sw.ezb_FIF==false && !ezbloaded) // Except for FIF page view load should only fly to ezb account
		ezb_s_account="hphqglobal,devhphqeasybuyv2";
	
	if(ezb_s_account!="hphqglobal,devhphqeasybuyv2" && ezb_s_account!="devhphqeasybuyv2"){
		if(_sw.s_account&&_sw.s_account.indexOf("hphq")!=-1){//if HP page use all Omniture accounts
			if(s_account=="hphqglobal" && s_account_mem.indexOf("hp")!=-1){
				ezb_s_account=s_account_mem+",devhphqeasybuyv2";
			}else{
				ezb_s_account=s_account+",devhphqeasybuyv2";
			}	
		}
	}
