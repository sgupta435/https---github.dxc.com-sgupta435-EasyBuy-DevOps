...
//Added by Nithiyanandhan on 06-Sep-2016 - Country/Language Mappings- Start
...
else
{
	...
	//Nithiyanandhan display both market segment code - start here
		    ...
		    /*$marketsegment=array();
			if(file_exists("../../$cc/$lc/".$cc."_".$lc."_config_smb.xml"))
			{
				
				array_push($marketsegment,"smb");
			}
			
			if(file_exists("../../$cc/$lc/".$cc."_".$lc."_config_hho.xml"))
			{
				array_push($marketsegment,"hho");
			}*/
			...
			foreach($return->p as $p) 
			{
					if ($p->r[0]) 
					{
						...
						foreach($p->r as $r) 
						{
							...
							$ctasegment = $r->n;
				         	if(($ctasegment!='Marketplace')&&($ctasegment!='JPOnlinestore'))
							{
								if($p["pi"] !="")
								{
									...
									if($cc=="us")
										{
											$r->u="http://marketplace.hpe.com/";
										}
										else if($cc=="in" || $cc=="nz")
										{
											$r->u="http://marketplace.hpe.com/home?country=".strtoupper($cc)."&amp;locale=".$lc;
										}
										else if ( $cc=="uk")
										{
											$r->u="https://www.hpe.com/uk/en/solutions/smb/jrit-promotions.html";
											$p->addAttribute("cta","chatOnline");
									        $p->addAttribute("ctalocalize","Chat online");

										}
										else if ( $cc=="jp")
																	{
											$p->r->addChild("u","https://www.hpe.com/jp/ja/japan/contact/sales.html");											
											$p["cta"]="contactUs";
						                    $p["ctalocalize"]="お問い合わせ";
											
										}
										/*else if ( $cc=="jp")
										{
											$p->r->addChild("u","http://h50146.www5.hpe.com/directplus_ent/");											
											$p["cta"]="buyNow";
						                    $p["ctalocalize"]=$ctaLocalizeBuynowText;
											
}
										*/										
				//Added by Ahteshem on 4/28/2017 - Find a partner button for all EMEA Countries except UK
										
										else if($cc=="at" || $cc=="cz" || $cc=="de"||$cc=="dk" || $cc=="es" || $cc=="fi"||$cc=="fr" || $cc=="ie" || $cc=="it"|| $cc=="nl" || $cc=="no" || $cc=="pl"||$cc=="pt" || $cc=="ru" || $cc=="se"||$cc=="tr" || $cc=="il"|| $cc=="me")
										{
							...
							{
//Japan Store change   -----Ahtesham 22January 2018----START                         
							if($cc=="jp")
								{
									//$r->u = $r->u . $spliturl;
								//$r->u = str_replace("&amp;","&",$r->u);
								...
								//$r->u = urldecode($r->u);
									...
									$pos = strrpos($spliturl, '=');
                                $r->u = $pos === false ? $spliturl : substr($spliturl, $pos + 1);
								//$r->u = $r->u . $spliturl;
								//$r->u = str_replace("&amp;","&",$r->u);
								$r->u = str_replace(";","",$r->u);
								$r->u = urldecode($r->u);
								}					
							 //Japan Store change   -----Ahtesham 22January 2018----END                         							
												
														
							
                                                         else  if ($cc !== "us" && $cc !== "in" && $cc !== "nz")
							{	$spliturl = $spliturl.";";
           ...
           else //if (($cc=="us") &&($ms=="all" || $ms=="smb" || $ms=="hho"))
							{
						               ...
						               if ( $cc=="uk")
										{
									        ...
									        $p->addAttribute("ctalocalize","Chat online");
											$p->addChild("r","");
											$p->r->addChild("u","https://www.hpe.com/uk/en/solutions/smb/jrit-promotions.html");
										...
										{
											//$r->u="";
											//$p->r->addChild("u","http://marketplace.hpe.com/");
							                $p["cta"]="contactUs";
						                    $p["ctalocalize"]="お問い合わせ";
							 $p->r->addChild("u","https://www.hpe.com/jp/ja/japan/contact/sales.html");												    
			...
			foreach($return->p as $p) 
			{
				if (!$p->r[0]) 
				{
					...
					if($p["pi"]!="")
					{
                                         	if($cc=="us"||$cc=="in" || $cc=="nz")
						{
						$p["pi"]="";
					        $p["cta"]="findLocalPartner";
						$p["ctalocalize"]=$ctaLocalizeResellerText;
						$p->r->addChild("u","https://findapartner.hpe.com/");
						}
					/*	if($cc=="us")
						{
						...
						{
							$p->r->addChild("u","http://marketplace.hpe.com/home?country=".strtoupper($cc)."&amp;locale=".$lc);
							$p["cta"]="buyNow";
						    $p["ctalocalize"]=$ctaLocalizeBuynowText;
						}*/
					...
					else
					{
						$p->r->addChild("u","https://findapartner.hpe.com/");
                                 ...
                                 $p["pn"]=str_replace("%23","#",$p["pn"]);
				 if( strpos($p["pn"], '#') !== false )
				{
					//$p->r="";
              ...
              //----START------Show only very first available Variant SKU if base SKU doesn't have marketplace price/URL-----Added by Ahtesham Khan
	      // 1. Generating an key pair index for listing all the products.		
   
	  $check_arr = array();
	  $cnt = 0;
	  foreach($ezbsimple as $key=>$ezbsimple_data)
	  {
		  //print_r($ezbsimple_data);
		  $check_arr[$cnt]['index'] = $cnt; 
		  $hpe_partnmbr =  (array) $ezbsimple_data{0}['pn']{0};
		  $check_arr[$cnt]['pn']  = $hpe_partnmbr[0];
		  $hpe_marketplace_source =  (array) $ezbsimple_data->r['t'][0];
		  $check_arr[$cnt]['t'] = $hpe_marketplace_source[0];
		  $hpe_marketplace_price =  (array) $ezbsimple_data->r['pr'][0];
		  $check_arr[$cnt]['pr'] = $hpe_marketplace_price[0];

		$cnt++;  
	  }
	
	
	// 2. Checking the partnumber and splitting the array based on it.
	  
		  $pn_array = array();
		  
		  foreach($check_arr as $key_check_arr=>$val){
			
					$pn_split = split("#",$val["pn"]);
					$pn_array[$pn_split[0]][$key_check_arr] = $val;
			
		  }
		// print_r($pn_array);
		foreach($pn_array as $key_check_arr=>$row_check_arr){
			if(count($row_check_arr) > 1 ) {
			   //print_r($row);
			
			foreach($row_check_arr as $each_row_check_arr) {
				  
			   
					$partnr_key[$key_check_arr] =  $each_row_check_arr['index'];
					if($each_row_check_arr['t'] == 'ebus')break;
					
					
				
			}
			}else{
				foreach($row_check_arr as $row_check_arr_ev) {
				$partnr_key[$key_check_arr] = $row_check_arr_ev['index'];
				}
			}
		}

       // 3. prepare 2 array for matching the items to be removed.
		$list1 = array_values($partnr_key);
		$list2 = array_keys($check_arr);
		
		
	
		
      // 4. Remove the unwanted PN on the arrays. Then add attribute for the truncated one.

	foreach($list2 as $remove_items)
	{
	
		 if(!(in_array($remove_items,$list1)))
	{
		
		$ezbsimple->p{$remove_items}[0]["pn"]="";
		$ezbsimple->p{$remove_items}[0]["cmpy"]="";
		$ezbsimple->p{$remove_items}[0]["sc"]="";
		$ezbsimple->p{$remove_items}[0]["pi"]="";
		$ezbsimple->p{$remove_items}[0]["cta"]="";
		$ezbsimple->p{$remove_items}[0]["ctalocalize"]="";
		$ezbsimple->p{$remove_items}[0]["nop"]="";
		
		
		unset($ezbsimple->p{$remove_items}[0]->r);
		$ezbsimple->p{$remove_items} = '';
		
		
	}
		else if( strpos($ezbsimple->p{$remove_items}[0]["pn"], '#') !== false ) 
	{
		 $base_skuFor_split =  $ezbsimple->p{$remove_items}[0]["pn"];
		 $ezbsimple->p{$remove_items}->addAttribute('vSku',$base_skuFor_split);
		 $base_sku= split("#",$ezbsimple->p{$remove_items}[0]["pn"]);
		 $ezbsimple->p{$remove_items}[0]["pn"]=$base_sku[0];
	}
		/*else{
		 $base_skuFor_split =  $ezbsimple->p{$remove_items}[0]["pn"];
		 $base_sku= split("#",$base_skuFor_split);
		 
	     $ezbsimple->p{$remove_items}->addAttribute('baseSku',$base_sku[0]);
	   }*/
	}
	      
 //----END------Show only very first available Variant SKU if base SKU doesn't have marketplace price/URL
