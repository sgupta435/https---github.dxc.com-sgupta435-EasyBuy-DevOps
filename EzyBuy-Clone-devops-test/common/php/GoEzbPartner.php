 ...
 * Include Get message
 */
require_once "include/displaymessage.php.inc";
/*
 * Process parameters
 */
...
if($param_ok){
	...
	if($url!=null){
	 	 	...
	 	 	/* Added by Sunetra Mhaske on 29/10/2015 - Message  */
		if($lang!=null and $client_id=="SureSupply")
			{
			echo displaymessage($lang);
			}
	/* Code end for message */	
	...
	//		if (!refererUrl) { refererUrl = document.referrer; }
				...
				if(document.cookie.indexOf(\"_ezb_o\")==-1){
					...
					document.cookie = \"_ezb_o=\"+\"$client_id\"+\";path=/;domain=.hpe.com;expires=\"+expiryDate;
				}
	 ...
	 */
			...
			if($mp2b_partnr!="" && strpos($mp2b_partnr, "__")===false){
			 //Added by Sunetra on 10/12/2014 - CDW issue - start
						/*  commented by sunetra mhaske 04 march as it is only decoding to US country not working for other countries 
					if($country=="us") { */
			...
			*/	
			...
			$command="window.setTimeout('window.top.location=\"'+'$goto'+'\"',500);";
			if(!$debug){
				echo "
					<script>
					$command;
